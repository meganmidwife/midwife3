...
export const BlogDisplay = async ({ id }: BlogDisplayProps) => {
  ...
  const truncatedDescription = asText(blog.data.description).substring(0,150) + ' ...'
  return (
  ...
  <Image src={"/meganThumbnail.png"} width={100} height={100} alt="megan" className="absolute inset-left-0 inset-top-0 -mt-1 ml-1 z-30 border-2 border-white/50 rounded-full w-12 h-12 md:w-24 md:h-24" />
    ...
    >
      ...
      >
        ...
        <div className="mb-10">
          <p className=" text-lg text-gray-300 bg-gray-800/50 p-4">{truncatedDescription} <TransitionLink document={blog} className="text-center z-30  mt-4 text-gray-100 hover:text-gray-400">More</TransitionLink>
        </p>
        </div>
        <TransitionLink document={blog} className="absolute max-w-1/2 text-center z-30 border-2 border-white bg-gray-300/20 hover:bg-gray-300/50 text-2xl text-gray-100 p-6">More</TransitionLink>
      </FadeIn>
      <FadeIn
